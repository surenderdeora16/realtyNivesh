...
const getPropertyHeadings = (data) => {
    ...
    // Loop through residential properties
    Object.values(data.residential).forEach(property => {
        headings.push(property.heading);
    });

    // Loop through commercial properties
    Object.values(data.commercial).forEach(property => {
        headings.push(property.heading);
    });
...
const EnquiryForm = ({ formType, heading, propertyLogo, isOpen, onClose = () => { }, eventSource = '', onFormSubmit = () => { }, funcParameter }) => {
    ...
    const headings = getPropertyHeadings(propertyData[0]);
    ...
    return (
        <div className='w-full h-screen bg-[#000000bd] backdrop-blur fixed top-0 left-0 z-[999999]'>
            <div className='w-full h-full flex justify-center items-center'>
                ...
                >
                    <div style={{ backgroundImage: `url('${process.env.basePath}/images/SUSHMAicon.png')` }} className='relative w-full h-full pt-7 lg:pt-16 pb-7 lg:pb-16  bg-no-repeat bg-left-bottom'>
                            ...
                            >
                                {({ values, handleSubmit, setFieldValue }) => (
                                    <Form onSubmit={handleSubmit} className={`h-full overflow-y-auto pr-2 ${showOtpInput ? 'self-center justify-self-center w-full cmd:w-1/2 ' : ''}`}>
                                        {!showOtpInput && (
                                            <div className={`${thankYou ? 'hidden' : ''} w-full h-full flex flex-col justify-between cmd:justify-center lg:justify-start ${formType === 'bookSiteVisit' ? 'gap-y-2' : 'gap-y-3 xl:gap-y-5'} `}>
                                            ...
                                            <div className={`${thankYou ? 'hidden' : ''} cxs:pt-5 lg:pt-0 lg:pl-8 2xl:pl-0 3xl:pl-16 w-full h-full flex flex-col justify-start`}>
                                                ...
                                                <div className='mt-3 xl:mt-4'>
                                                    ...
                                                    <button
                                                        ...
                                                        className='mt-16 cmd:mt-16 w-full h-[40px] bg-[#2E72A9] rounded-[20px] text-[#fff] font-supera500 tracking-wide text-[16px]'>
                                                        {loaderShow ? <div class="lds-ellipsis"><div></div><div></div><div></div><div></div></div> : `${action == 'verifyOTP' ? 'Submit' : 'Verify'}`}
                                                    </button>
                                                </div>
                                            </div>
                                        )}
                                        ...
                                        {thankYou && (
                                            <div className='cxs:pt-5 lg:pt-0 lg:pl-8 xl:pl-16 w-full h-full flex flex-col justify-start'>
                                                <div className='mt-5 flex flex-col gap-10 w-full h-full'>
                                                    ...
                                                    <h6 className='mt-5 text-[17px] font-supera500 text-[#514E4E]'>or call us at <button onClick={() => window.location.href = 'tel:918968066698'} className='text-[#E98F0A] cursor-pointer'>+91 99880 10405</button></h6>
                                                </div>
                                            </div>
                                        )}
                                    </Form>
