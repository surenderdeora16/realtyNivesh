...
import AgentImg from '../../assets/images/agenticon.png'
    ...
    useEffect(() => {
        if (isHidden) {
            const timer = setTimeout(() => {
                setIsHidden(false); // Make content visible again after 10 seconds
            }, 10000);
            setTimerId(timer); // Save the timer id so it can be cleared if needed
            ...
            <div className='w-screen h-full relative '>
                ...
                <div className='fixed bottom-20 sm:bottom-5 md:bottom-10 right-2  md:right-10 z-50'>
                    <div className='relative flex  flex-col justify-center items-center '>
                        ...
                        <h6 className={`scrollingText ${open || isHidden ? 'opacity-0 duration-500 hidden' : 'opacity-100 duration-500'} pb-3.5 max-w-[150px] text-[14px] text-[#474747] font-supera800 text-center`}>
                            Need immediate assistance?
                        </h6>
                        {!open && (
                            <div onClick={() => setOpen(true)} className={`${isHidden ? 'opacity-0 duration-500 hidden' : 'opacity-100 duration-500'} call-animation relative z-50 cursor-pointer size-[65px] rounded-full`}>
                        ...
                        <div className={`mt-3 ${open || isHidden ? 'opacity-0 duration-500 hidden' : 'opacity-100 duration-500'}`}>
                            <strong className='font-supera700 text-[10px] text-center capitalize text-[#3D6B96]'>Pawan Kumar</strong>
                        ...
                        <div className={`${open ? ` opacity-100 w-[300px] whitespace-nowrap  xs:w-[353px]  duration-1000` : `w-[0px]  opacity-0 whitespace-nowrap duration-300`} duration-500 shadow-lg absolute bottom-[90%] -right-[0px] xs:right-8 z-50   bg-white   rounded-xl overflow-hidden `}>
                            <div className='w-full h-full relative after:absolute after:w-[60%] after:h-[4px] after:bottom-0 after:left-[50%] after:translate-x-[-50%] after:rounded-t-2xl after:bg-[#1E6DA4]'>
                                ...
                                <div className='py-4  px-3 xs:px-[37px]  relative flex flex-col gap-6'>
                                    <h5 className=' text-[16px] text-center text-[#535353] font-supera800'>Need immediate assistance?</h5>
                                    <div className='flex'>
                                        <div className={`relative z-50 ${open ? 'cursor-pointer' : 'cursor-auto'} size-[65px]   rounded-full`}>
                                            <Image src={AgentImg} className='relative z-50 w-full h-full' alt="agent-img" />
                                        </div>
                                        <div className='ml-4'>
                                            <strong className='font-supera800 text-[15px] text-center capitalize text-[#1e6da4]'>Pawan Kumar</strong>
